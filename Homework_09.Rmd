...
run_stats <- function(data = NULL, n = sample(20:80, 1)) {
  ...
  ano_stats <- list(f_pval <- unlist(z)[9])
  return(ano_stats)
}
...
d_frame <- create_dataset(n = n)

p_val <- run_stats(data = d_frame, n = n)

boxplot <- graph_data(data = d_frame) # create boxplot

print(boxplot) # print boxplot
print(p_val) # print p-value
```
...
I modified my *run_stats* function to run a post-hoc Tukey HSD test to compare all four groups to each other, i.e. test for significance for all possible comparisons of two groups.
...
stats <- run_stats(data = d_frame, n = n)
