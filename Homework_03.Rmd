...
REPLACE: \2 \1 (\3)
```
Explanation: I captured the first name, last name, and institution from the original data frame, and replaced them in the correct order, adding a pair of parentheses around the institution. 
...
REPLACE: \1\n
```
Explanation: I captured each tune by finding the .mp3 part of the file name, and any spaces after it, and replaced with each file name followed by a single line break, placing each on its own line.
...
REPLACE: \2_\1\3
```
Explanation: I captured the four digit number for each file by searching for a sequence of exactly four single number characters, and then I captured the song title and the .mp3 part of every file name. For my replace, I rearranged these three components and placed an underscore between the song title and four digit number. 
